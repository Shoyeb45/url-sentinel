...
const URLSentinelDashboard = () => {
  ...
  const filteredLogs = logs.filter(log => {
    const matchesSearch = !searchTerm || 
      ...
      log.uri_path?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const matchesThreat = filterThreat === 'all' || log.attack_type === filterThreat;
  ...
  const getThreatBadge = (attackType, confidence) => {
  ...
  const uniqueThreats = [...new Set(logs.map(log => log.attack_type).filter(Boolean))];
      ...
      <div className="max-w-[1600px] mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={analyzeLogs} variant="default" size="sm" disabled={loading || logs.length === 0}>
              Analyze All Logs
            </Button>
          </div>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-900/80 border-b border-slate-800">
                    ...
                    <th className="text-left px-4 py-3 font-semibold text-slate-200 text-sm whitespace-nowrap">Status</th>
                    <th className="text-left px-4 py-3 font-semibold text-slate-200 text-sm whitespace-nowrap">Attack Type</th>
                ...
                <tbody className="divide-y divide-slate-800/50">
                  {loading ? (
                    <tr>
                      <td colSpan={9} className="text-center py-16 text-slate-400 bg-slate-900/20">
                    ...
                    filteredLogs.map((log, idx) => (
                      <tr 
                        ...
                        className={`
                          ...
                          hover:bg-slate-800/50 transition-all duration-150 cursor-pointer
                        `}
                      >
                        ...
                        <td className="px-4 py-4 font-mono text-xs text-slate-400 max-w-[300px]">
                          <div className="truncate" title={log.uri_path}>
                            {log.uri_path || 'N/A'}
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4">
                          {getThreatBadge(log.attack_type, log.confidence)}
                        </td>
                        <td className="px-4 py-4">
                          {log.confidence ? (
                            <div className="flex items-center gap-3 min-w-[140px]">
                              <div className="flex-1 bg-slate-700/50 rounded-full h-2.5 overflow-hidden">
                                <div
                                  className={`h-full rounded-full transition-all duration-300 ${
                                    parseFloat(log.confidence) > 80
                                      ? 'bg-gradient-to-r from-red-500 to-red-600'
                                      : parseFloat(log.confidence) > 50
                              ...
                              <span className="text-xs font-semibold text-slate-300 min-w-[45px]">
                                {parseFloat(log.confidence).toFixed(1)}%
                              </span>
                            </div>
                          ) : (
